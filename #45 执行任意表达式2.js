...
const execute = (template, data) => {
    return eval(`with(data){` + template + `}`)
}

const execute = (template, data) => {
    let param = Object.getOwnPropertyNames(data)
    return new Function(...param, 'template', `return eval(template)`)(...param.map(name => data[name]), template);
}